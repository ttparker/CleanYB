...
for dataFile in dataFiles
  ...
  if dataFile == dataFiles[end] && summarySection  # use summary section from last file to fill out system.params
    hamParams = split(readline(f), r": |, ")[[2, 4, 6, 8, 10]]
    system.params.Jzz, system.params.Jpm, system.params.Jpmpm, system.params.Jzpm = map(x -> parse(Float64, x), hamParams[1:4])
    system.params.L = parse(Int64, hamParams[5])
